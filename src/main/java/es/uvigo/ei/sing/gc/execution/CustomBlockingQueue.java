...
class CustomBlockingQueue extends AbstractQueue<Runnable> implements BlockingQueue<Runnable>, Serializable {
		...
		public CustomBlockingQueue() {
			this.tasksByUser = new HashMap<String, Queue<Runnable>>();
		...
		public void beginBatchAdd(String userId) {
			this.writeLock.lock();
		...
		@Override
		public boolean offer(Runnable runnable) throws IllegalArgumentException {
		...
		public Runnable poll(long timeout, TimeUnit unit)
			...
			try {
				Date until = new Date(System.currentTimeMillis() + unit.toMillis(timeout));
				while (
					this.isEmpty() && 
					this.emptyCondition.awaitUntil(until)
				);
		...
		public int drainTo(Collection<? super Runnable> c) {
			...
			try {
				...
				if (!this.isEmpty()) {
					for (Queue<Runnable> tasks : this.tasksByUser.values()) {
						if (c.addAll(tasks)) {
		...
		public int drainTo(Collection<? super Runnable> c, int maxElements) {
			...
			try {
				...
				if (!this.isEmpty()) {
					for (Map.Entry<String, Queue<Runnable>> taskEntry : this.tasksByUser.entrySet()) {
			...
			return new Iterator<Runnable>() {
					...
					try {
						if (this.currentIterator == null || !this.currentIterator.hasNext()) {
							if (this.userIds.hasNext()) {
								...
								this.currentIterator = CustomBlockingQueue.this.tasksByUser
									.get(this.currentUserId).iterator();
							} else {
				...
				public void remove() {
					...
					try {
						...
						final Queue<Runnable> queue = 
							CustomBlockingQueue.this.tasksByUser.get(this.currentUserId);
